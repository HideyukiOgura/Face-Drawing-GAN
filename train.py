...
######## loss weights
parser.add_argument("--condGAN", type=float, default=2.0, help="weight of the adversarial style loss")
parser.add_argument("--cond_clip", type=float, default=1.0, help="weight of the semantic loss")
parser.add_argument("--cond_paired", type=float, default=2.0, help="weight of the paire loss")
...
parser.add_argument('--cos_clip', type=int, default=1, help='use cosine similarity for CLIP semantic loss')

### save options
parser.add_argument('--save_epoch_freq', type=int, default=100, help='how often to save the latest model in steps')#1000
parser.add_argument('--slow', type=int, default=0, help='only frequently save netG_A')
parser.add_argument('--log_int', type=int, default=50, help='display frequency for tensorboard')#50
...
netD_B = networks.define_D(D_input_nc_B, opt.ndf, opt.netD, opt.n_layers_D, opt.norm, use_sigmoid=True)
...
train_data = PairedDataset(opt.dataroot, opt.root2, opt, transforms_r=transforms_r, 
                mode=opt.mode)
...
for epoch in range(opt.epoch, opt.n_epochs):
    for i, batch in enumerate(dataloader):
        ...
        condGAN = opt.condGAN
        cond_clip = opt.cond_clip
        cond_paired = opt.cond_paired

        #################### Generator ####################


        fake_B = netG_A(real_A) # G_A(A)
        loss_paired = criterion_MSE(fake_B, real_B)


        # GAN loss D_B(G_A(A))
        loss_GAN = 0
        loss_GAN = criterionGAN(netD_B(fake_B), True)

        # combined loss and calculate gradients
        loss_G = condGAN*loss_GAN + cond_paired*loss_paired
        ...
        if opt.output_nc == 1:
            ...
            recog_style0 = (recog_style - 0.48145466) / 0.26862954
            recog_style1 = (recog_style - 0.4578275) / 0.26130258
            recog_style2 = (recog_style - 0.40821073) / 0.27577711
            recog_style = torch.cat([recog_style0, recog_style1, recog_style2], dim=1)  
                
        patches_r = [torch.nn.functional.interpolate(recog_real, size=224)]  #The resize operation on tensor.
        patches_s = [torch.nn.functional.interpolate(recog_style, size=224)]
        patches_l = [torch.nn.functional.interpolate(line_input, size=224)]

        ## patch based clip loss
        if opt.N_patches > 1:
            patches_r2, patches_s2, patches_l2 = createNRandompatches(recog_real, recog_style,line_input, opt.N_patches, opt.patch_size)
            patches_r += patches_r2
            patches_s += patches_s2
            patches_l += patches_l2

        loss_clip = 0
        myloss_recog_real = 0
        myloss_recog_style = 0
 
        for patchnum in range(len(patches_r)):

            real_patch = patches_r[patchnum]
            style_patch = patches_s[patchnum]
            ...
            feats_s = clip_model.encode_image(style_patch).detach()
            feats_line = clip_model.encode_image(line_patch)
            myloss_recog_real += patch_factor * torch.clone(criterionCLIP(feats_line, feats_r))
            myloss_recog_style += patch_factor * torch.clone(criterionCLIP(feats_line, feats_s))
            
        myloss_clip_real = torch.mean(myloss_recog_real)
        myloss_clip_style = torch.mean(myloss_recog_style)
        
        loss_clip += 0.5 * myloss_clip_real + myloss_clip_style
        
        loss_G += cond_clip* loss_clip
        ...
        if (i+1)%opt.log_int==0:
            ...
            errors['loss_paired'] = torch.mean(loss_paired) if not isinstance(loss_paired, (int,float)) else loss_paired
            ...
            errors['loss_clip'] = torch.mean(loss_clip) if not isinstance(loss_clip, (int,float)) else loss_clip
            errors['loss_clip_real'] = torch.mean(myloss_clip_real) if not isinstance(myloss_clip_real, (int,float)) else myloss_clip_real
            errors['loss_clip_style'] = torch.mean(myloss_clip_style) if not isinstance(myloss_clip_style, (int,float)) else myloss_clip_style
